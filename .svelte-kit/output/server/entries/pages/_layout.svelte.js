...
const Header = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_page();
  return `<header class="svelte-1b1ge90"><div class="corner svelte-1b1ge90" data-svelte-h="svelte-19vg9y6"><a href="/" class="svelte-1b1ge90"><img${add_attribute("src", ta_logo, 0)} alt="Tactical Athletics" class="svelte-1b1ge90"></a></div> <div class="header"><div class="signedInStatus"><p class="nojs-show loaded">${$page.data.session ? `${$page.data.session.user?.username ? `${escape(user.username = $page.data.session.user?.username)}` : ``} ${escape(user.name = $page.data.session.user?.name)} ${$page.data.session.user?.image ? `<span style="${"background-image: url('" + escape($page.data.session.user.image, true) + "')"}" class="avatar"></span>` : ``} <span class="signedInText"><small data-svelte-h="svelte-qavuiv">Signed in as</small><br> <strong>${escape($page.data.session.user?.name)}</strong></span> <a href="/auth/signout" class="buttonPrimary svelte-1b1ge90" data-svelte-h="svelte-m13ys5">Sign out</a>` : `<a href="/auth/signin" class="buttonPrimary svelte-1b1ge90" data-svelte-h="svelte-1o43lvr">Sign in</a>`}</p></div>   ${$page.data.session ? `${$page.data.session.user?.name ? `<nav class="svelte-1b1ge90"><svg viewBox="0 0 2 3" aria-hidden="true" class="svelte-1b1ge90"><path d="M0,0 L1,2 C1.5,3 1.5,3 2,3 L2,0 Z" class="svelte-1b1ge90"></path></svg> <ul class="svelte-1b1ge90"><li class="${[
    "svelte-1b1ge90",
    $page.url.pathname.includes("/experience") ? "active" : ""
  ].join(" ").trim()}"><a href="${"/experience/" + escape(todayString, true)}" class="svelte-1b1ge90">Experience</a></li> <li class="${[
    "svelte-1b1ge90",
    $page.url.pathname.includes("traininglog") ? "active" : ""
  ].join(" ").trim()}"><a href="${"/traininglog/" + escape(todayString, true)}" class="svelte-1b1ge90">Train</a></li> <li class="${["svelte-1b1ge90", $page.url.pathname.includes("foodlog") ? "active" : ""].join(" ").trim()}"><a href="${"/foodlog/" + escape(todayString, true)}" class="svelte-1b1ge90">Fuel</a></li> <li class="${["svelte-1b1ge90", $page.url.pathname === "/reference" ? "active" : ""].join(" ").trim()}" data-svelte-h="svelte-qrgmux"><a href="/reference" class="svelte-1b1ge90">Reference</a></li></ul> <svg viewBox="0 0 2 3" aria-hidden="true" class="svelte-1b1ge90"><path d="M0,0 L0,3 C0.5,3 0.5,3 1,2 L2,0 Z" class="svelte-1b1ge90"></path></svg> <div class="corner svelte-1b1ge90" data-svelte-h="svelte-125ddja"></div></nav>` : ``}` : ``}</div></header>`;
});
const css = {
...
const Layout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  $$result.css.add(css);
  return `<div class="app svelte-1pc322i">${validate_component(Header, "Header").$$render($$result, {}, {}, {})} <main class="svelte-1pc322i">${slots.default ? slots.default({}) : ``}</main> <footer class="svelte-1pc322i" data-svelte-h="svelte-1fus0uz"><a href="https://kit.svelte.dev" class="svelte-1pc322i">visit tactical athletics for more stuff like this</a></footer> </div>`;
});
export {
  Layout as default
};
