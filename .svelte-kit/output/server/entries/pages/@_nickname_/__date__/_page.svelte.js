...
<div class="container"><h3 class="svelte-eu6dzx"><a data-sveltekit-reload href="${"/@" + escape(data.nickname, true) + "/"}">@${escape(data.nickname)} Food Porn</a> 
...
<div class="container"><div class="left-image"><a data-sveltekit-reload href="${"/@" + escape(data.nickname, true) + "/" + escape(yesterdayString, true)}"><i class="fa fa-arrow-circle-left" style="font-size:36px; color:blue; padding:10px"></i></a></div>
	<div class="foodheader">What ${escape(data.nickname)} ate on ${escape(todayString)}</div>
	<div class="right-image"><a data-sveltekit-reload href="${"@" + escape(data.nickname, true) + "/" + escape(tomorrowString, true)}"><i class="fa fa-arrow-circle-right" style="font-size:36px; color:blue; padding:10px"></i></a></div></div>



<div class="grid svelte-eu6dzx">
	${each(data.todaysFoods, (todaysFood, i) => {
    return `${todaysFood.foodReference != null ? `<div class="grid-item">
				<input type="hidden"${add_attribute("value", JSON.stringify((({ imageBase64, ...rest }) => rest)(todaysFood.foodReference)), 0)} name="food" class="svelte-eu6dzx">
				
				<button${add_attribute("title", todaysFood.foodReference.food_name, 0)} class="svelte-eu6dzx"><img src="${"data:image/png;base64, " + escape(todaysFood.foodReference.imageBase64, true)}"${add_attribute("title", todaysFood.foodReference.food_name, 0)} class="food_image svelte-eu6dzx"></button>		
			</div>` : ``}`;
  })}</div>

<center><div class="experienceheader">What ${escape(data.nickname)} eats most frequently</div></center>


<div class="grid svelte-eu6dzx">
	${each(data.foodReferences, (foodReference, i) => {
    return `${foodReference.foodReference != null ? `<div class="grid-item">
				<input type="hidden"${add_attribute("value", JSON.stringify((({ imageBase64, ...rest }) => rest)(foodReference.foodReference)), 0)} name="food" class="svelte-eu6dzx">
				
				<button title="${escape(foodReference.foodReference.food_name, true) + " (used " + escape(foodReference.count, true) + " times)"}" class="svelte-eu6dzx"><img src="${"data:image/png;base64, " + escape(foodReference.foodReference.imageBase64, true)}" alt="${escape(foodReference.foodReference.food_name, true) + " (used " + escape(foodReference.count, true) + " times)"}" class="food_image svelte-eu6dzx"></button>		
			</div>` : ``}`;
  })}</div>
