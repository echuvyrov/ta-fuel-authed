...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.data === void 0 && $$bindings.data && data !== void 0)
    $$bindings.data(data);
  $$result.css.add(css);
  return `<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">   <div class="container"><div class="left-image"><a data-sveltekit-reload href="${"/experience/" + escape(yesterdayString, true)}"><i class="fa fa-arrow-circle-left" style="font-size:36px; color:blue; padding:10px"></i></a></div> <div class="foodheader">What you ate on ${escape(todayString)}</div> <div class="right-image"><a data-sveltekit-reload href="${"/experience/" + escape(tomorrowString, true)}"><i class="fa fa-arrow-circle-right" style="font-size:36px; color:blue; padding:10px"></i></a></div></div>  <div class="grid svelte-13ewkjp"> ${each(data.todaysFoods, (todaysFood, i) => {
    return `${todaysFood.foodReference != null ? `<form action="?/addfood" method="POST"><div class="grid-item"> <input type="hidden"${add_attribute("value", JSON.stringify((({ imageBase64, ...rest }) => rest)(todaysFood.foodReference)), 0)} name="food" class="svelte-13ewkjp">  <button${add_attribute("title", todaysFood.foodReference.food_name, 0)} class="svelte-13ewkjp"><img src="${"data:image/png;base64, " + escape(todaysFood.foodReference.imageBase64, true)}"${add_attribute("title", todaysFood.foodReference.food_name, 0)} class="food_image svelte-13ewkjp"> </button></div> </form>` : ``}`;
  })}</div> <center data-svelte-h="svelte-1btd812"><div class="experienceheader">What you eat most frequently (simply click to add to today&#39;s log)</div></center>  <div class="grid svelte-13ewkjp"> ${each(data.foodReferences, (foodReference, i) => {
    return `${foodReference.foodReference != null ? `<form action="?/addfood" method="POST"><div class="grid-item"> <input type="hidden"${add_attribute("value", JSON.stringify((({ imageBase64, ...rest }) => rest)(foodReference.foodReference)), 0)} name="food" class="svelte-13ewkjp">  <button title="${escape(foodReference.foodReference.food_name, true) + " (used " + escape(foodReference.count, true) + " times)"}" class="svelte-13ewkjp"><img src="${"data:image/png;base64, " + escape(foodReference.foodReference.imageBase64, true)}" alt="${escape(foodReference.foodReference.food_name, true) + " (used " + escape(foodReference.count, true) + " times)"}" class="food_image svelte-13ewkjp"> </button></div> </form>` : ``}`;
  })}</div> `;
});
export {
  Page as default
