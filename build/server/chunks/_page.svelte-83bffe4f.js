import { c as create_ssr_component, o as onDestroy, v as validate_component, e as escape, d as add_attribute, j as compute_rest_props, k as createEventDispatcher, h as each, l as spread, p as escape_object, q as escape_attribute_value, t as compute_slots } from './index3-d06080d5.js';
import { d as derived } from './index2-4f7f68c2.js';
...
function requireFuzzy () {
	if (hasRequiredFuzzy) return fuzzy$1.exports;
	hasRequiredFuzzy = 1;
	...
	} (fuzzy$1, fuzzy$1.exports));
	return fuzzy$1.exports;
}

var fuzzyExports = requireFuzzy();
var fuzzy = /*@__PURE__*/getDefaultExportFromCjs(fuzzyExports);
...
const createStore = () => {
  ...
  const switch1193Provider = async ({
    ...
    assign({
      connected: true,
      selectedAccount: Array.isArray(accounts) && accounts.length ? accounts[addressOrIndex] : null,
      chainId,
      accounts
    });
    emit();
  ...
  const init = () => {
    loadWeb3();
    if (!Web3.version)
      throw new Error("[svelte-web3] Cannot find Web3");
  ...
  const set1193Provider = async (eipProvider, addressOrIndex) => {
    ...
    const web32 = new Web3(eipProvider);
    assign({
      web3: web32,
      eipProvider,
  ...
  const setProvider = async (provider, addressOrIndex = 0) => {
    ...
    const chainId = alwaysNumber(await web32.eth.getChainId());
    let accounts = [];
    try {
...
const createContractStore = () => {
  const { emit, get, subscribe, assign, deleteAll } = proxied();
  const attachContract = async (name, address2, abi2, options) => {
    assign({
      [name]: [address2, abi2, options]
    });
    emit();
  };
  return {
...
const makeEvmStores = (name) => {
  ...
  allStores[name].walletType = derived(evmStore, ($evmStore) => {
    if (!$evmStore.eipProvider)
      return null;
    if (typeof $evmStore.eipProvider === "string")
      return $evmStore.eipProvider;
    if ($evmStore.eipProvider.isNiftyWallet)
      return "Nifty";
    if ($evmStore.eipProvider.isTrust)
      return "Trust";
    if ($evmStore.eipProvider.isMetaMask)
      return "MetaMask (or compatible)";
    if ($evmStore.eipProvider.bridge && /walletconnect/.test($evmStore.eipProvider.bridge))
...
<div data-svelte-typeahead role="${"combobox"}" aria-haspopup="${"listbox"}" aria-controls="${escape(id, true) + "-listbox"}"${add_attribute("aria-expanded", showResults, 0)} id="${escape(id, true) + "-typeahead"}" class="${["svelte-1t4elht", results.length > 0 ? "dropdown" : ""].join(" ").trim()}"${add_attribute("this", comboboxRef, 0)}>${validate_component(Search, "Search").$$render(
      $$result,
      Object.assign(
        { id },
...
<div style="${"text-align:center; margin:25px auto; width:85%; max-width:1200px;"}"><form action="${"?/addfood"}" method="${"POST"}">${validate_component(Typeahead, "Typeahead").$$render(
	...
	<center><a data-sveltekit-reload href="${"/foodlog/" + escape(yesterdayString, true)}"><i class="${"fa fa-arrow-circle-left"}" style="${"font-size:36px; color:blue; padding:10px"}"></i></a>

		<div class="${"foodheader"}">Food Log for ${escape(todayString)}</div>
		
		<a data-sveltekit-reload href="${"/foodlog/" + escape(tomorrowString, true)}"><i class="${"fa fa-arrow-circle-right"}" style="${"font-size:36px; color:blue; padding:10px"}"></i></a></center>
	${`
...
//# sourceMappingURL=_page.svelte-83bffe4f.js.map
